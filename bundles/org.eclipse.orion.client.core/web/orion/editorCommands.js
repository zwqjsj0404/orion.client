		...
		generateEditorCommands: function(editor) {
			...
			if (!this.isReadOnly) {
				...
				if(this.inputManager.onSave){
					editor.getTextView().setAction("save", {name: "Save", handler: dojo.hitch(this, function () {
				...
				editor.getTextView().setAction("gotoLine", {name: "Go to Line", handler: dojo.hitch(this, function () {
				...
				dojo.when(getContentTypes(this.serviceRegistry), dojo.hitch(this, function() {
					for (var i=0; i<actionReferences.length; i++) {
						...
						if (info.key) {
							...
							textView.setAction(command.id, {name: command.name, handler: command.callback});
						}				
					}
				}));
			}
...
exports.UndoCommandFactory = (function() {
	...
	UndoCommandFactory.prototype = {
		createUndoStack: function(editor) {
			...
			editor.getTextView().setAction("undo", {name: "Undo", handler: function() {
				undoStack.undo();
				return true;
			}});
			...
			editor.getTextView().setAction("redo", {name: "Redo", handler: function() {
				undoStack.redo();
				return true;
			}});
