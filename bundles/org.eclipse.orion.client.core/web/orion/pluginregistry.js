			...
			try {
				if (message.method) {
					...
					if (message.error) {
						...
						if(_isTask(message)){
							_serviceRegistry.getService("orion.page.progress").followOperation(message.result.task).then(function(progressResp){
								if(progressResp.Result.Severity === "Ok") {
									deferred.resolve(progressResp.Result.JsonData);
								} else {
									deferred.reject(progressResp.Result);
								}
							});
						} else {
